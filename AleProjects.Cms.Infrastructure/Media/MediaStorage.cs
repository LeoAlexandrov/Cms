...
namespace AleProjects.Cms.Infrastructure.Media
{
	...
	public class MediaStorage(IConfiguration configuration)
	{
		...
		private static string FromBase64ToVirtual(string previewName)
		{
			string result;
		...
		private static bool DeletePreviews(string cachePath, IList<EntryNames> files, IList<EntryNames> folders)
		{
		...
		private static bool CheckSignature(byte[] header, string extension)
		{
			switch (extension)
			{
				case ".png":
		...
		public async ValueTask<MediaStorageEntry> Preview(string path, string previewPrefix, int size)
		{
			...
			string cachedName = string.Format("{0}_{1}x{1}.webp", System.Web.HttpUtility.UrlEncode(previewPrefix), size);
