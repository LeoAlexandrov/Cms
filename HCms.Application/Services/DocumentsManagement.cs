...
namespace HCms.Application.Services
{
	...
	public partial class ContentManagementService(CmsDbContext dbContext, FragmentSchemaRepo schemaRepo, IAuthorizationService authService, IEventNotifier notifier)
	{
		...
		private static int GetConsistentStatus(int currentStatus, int newStatus)
		{
			if (newStatus == (int)PublishStatus.Published || currentStatus == newStatus)
				return currentStatus;

			if (newStatus == (int)PublishStatus.InReview)
				return currentStatus == (int)PublishStatus.Published ? (int)PublishStatus.InReview : currentStatus;

			return (int)PublishStatus.Unpublished;
		}
		...
		public async Task<Result<DtoFullDocumentResult>> CreateDocument(DtoCreateDocument dto, ClaimsPrincipal user)
		{
			...
			if (dto.Parent > 0)
			{
				...
				path = PathJoin(parent.Path, slug);
		...
		public async Task<Result<DtoDocumentResult>> UpdateDocument(int id, DtoUpdateDocument dto, ClaimsPrincipal user)
		{
			...
			if (slugChanged)
				if (doc.Parent > 0)
				{
					...
					for (int i = 0; i < children.Length; i++)
					{
						children[i].Path = PathJoin(newPath, children[i].Path[l..]);
		...
		public async Task<Result<DtoDocumentResult>> SetParentDocument(int id, int parentId, ClaimsPrincipal user)
		{
			...
			for (int i = 0; i < children.Length; i++)
			{
				...
				child.Path = PathJoin(newPath, child.Path[l..]);
