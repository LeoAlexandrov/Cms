...
namespace HCms.Domain.Types
{
	...
	public class XSElement
	{
		...
		//[JsonIgnore]
		[XmlIgnore]
		public bool RepresentsContainer => Parent == null && Annotation.HasValue && Annotation.Value.IsContainer;

		//[JsonIgnore]
		[XmlIgnore]
		public bool RepresentsImage => Annotation.HasValue && Annotation.Value.IsImage;
		...
		public object DefaultObjectValue()
		{
			...
			return XmlType switch
			{
				...
				"boolean" => false,
				"integer" or "int" or "short" or "byte" => DefaultIntValue(),
				"decimal" or "double" or "float" => DefaultDoubleValue(),
				_ => string.Empty
			};
		}
		...
		public string DefaultXmlValue()
		{
			...
			return XmlType switch
			{
				"token" => FacetEnumeration != null && FacetEnumeration.Count > 0 ? FacetEnumeration[0] : string.Empty,
				"boolean" => "false",
				"integer" or "int" or "short" or "byte" => DefaultIntValue().ToString(),
				"decimal" or "double" or "float" => DefaultDoubleValue().ToString(new NumberFormatInfo()),
				_ => string.Empty
			};
		}
		...
		{
			if (nss.TryAdd(Namespace, prefix))
